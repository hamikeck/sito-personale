...
- **Dark mode** basata su classe: custom variant `@custom-variant dark (&:is(.dark *))` in `globals.css`; il tema scuro si attiva aggiungendo la classe `.dark` a un antenato (non c'è ancora un theme provider).
